...
Stack cerrado (no proponer alternativas sin un problema real que lo justifique): Recharts para gráficas, Supabase (Postgres + Auth), Claude API (`/v1/messages`) para el scoring, Vercel (front) + Railway (API) para deploy.
...
13. Antonio se pierde con facilidad en "¿esta línea en qué archivo, qué función, qué sitio exacto?" cuando se le da solo un fragmento de código suelto. Cuando le pidas cambiar algo, no basta con la línea aislada: dale también el archivo exacto (ruta completa) y, si hay cualquier duda posible de ubicación, el contenido completo del archivo tal como debería quedar (o con la zona a cambiar muy claramente delimitada), no solo el fragmento nuevo.
...
5. Después: alertas a Telegram, testing con Vitest, automatizar la ejecución periódica del scraper, migración a AWS.
